...
def main() -> int:
    parser = argparse.ArgumentParser(description="Recover missing MPFM daily/recon data from a folder of Daily PDFs.")
    parser.add_argument("--bank", required=True, help="Bank code, e.g. B08")
    parser.add_argument("--folder", required=True, help="Folder containing Daily PDF files")
    ...
    parser.add_argument("--force", action="store_true", help="Repair all parsed days even when daily/recon already exist")
    ...
    for pdf_path in sorted(folder.glob("*.pdf")):
        record = server.engine.parse_pdf(str(pdf_path), "daily")
        ...
        if args.date_to and production_date > args.date_to:
            continue
        parsed_records[production_date] = record

    if not parsed_records:
        raise SystemExit("No valid daily PDFs found for the selected date range.")

    status = _load_existing_status(bank, sorted(parsed_records))
    targets: list[tuple[str, dict]] = []
    for production_date in sorted(parsed_records):
        day_status = status.get(production_date, {"daily": 0, "recon": 0, "hourly": 0})
        needs_repair = args.force or day_status.get("daily", 0) == 0 or day_status.get("recon", 0) == 0
        if needs_repair:
            targets.append((production_date, parsed_records[production_date]))

    print(f"Parsed days: {sorted(parsed_records)}")
    print(f"Target days: {[day for day, _ in targets]}")
    if not targets:
        print("Nothing to repair.")
        return 0

    run_id = server.start_run("repair-mpfm-daily-folder", str(folder), server.DEFAULT_DENSITY, len(targets))
    workbook_payloads: dict[tuple[str, str], dict[str, object]] = defaultdict(lambda: {"sheets": {}, "area_rows": []})
    repaired_days: list[str] = []
    failed_days: list[dict[str, str]] = []

    try:
        for production_date, record in targets:
            ...
            daily_sheet = f"DAILY_{bank}_{day_tag}"
            recon_sheet = f"RECON_{bank}_{day_tag}"

            try:
                server._purge_mpfm_rows(bank, production_date, "daily")
                server._purge_mpfm_rows(bank, production_date, "recon")

                df_daily = server.engine.build_daily_df(record, bank)
                daily_rows = server.dataframe_to_records(df_daily)
                server.db_store_sheet_rows(run_id, excel_file, daily_sheet, daily_rows)
                ...
                bucket = workbook_payloads[(yr, mo)]
                bucket["sheets"][daily_sheet] = df_daily
                if df_recon is not None and not df_recon.empty:
                    bucket["sheets"][recon_sheet] = df_recon
                area_row = server.engine.build_area_totals_row(record, bank)
                if area_row:
                    bucket["area_rows"].append(area_row)

                server.db_add_issue(
                    run_id,
                    excel_file,
                    "daily_recovered_from_folder",
                    "info",
                    f"{bank}/{production_date}",
                    production_date,
                    f"Dados daily/recon recuperados a partir de {Path(record.get('pdf_path') or '').name}.",
                )
                repaired_days.append(production_date)
                print(f"repaired {production_date} -> {excel_file}")
        ...
        for (yr, mo), payload in workbook_payloads.items():
            ...
            server.engine._merge_excel(str(workbook_path), payload["sheets"], payload["area_rows"])
        ...
        server.finish_run(run_id, "error", {"bank": bank, "folder": str(folder), "repaired_days": repaired_days, "failed_days": failed_days})
        raise
    ...
    print("DONE")
    print({"repaired_days": repaired_days, "run_id": run_id})
    return 0
